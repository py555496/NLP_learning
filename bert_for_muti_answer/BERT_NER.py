...
flags.DEFINE_float("qp_loss_rate", 0.35, "The initial learning rate for Adam.")
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        :return:
        """
        return ["[PAD]","B-TAG", "I-TAG", "O", "E-TAG", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X","[CLS]","[SEP]"]
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    with open("./middle_data/label2id.pkl", 'wb') as w:
    ...
    if len(tokens) >= max_seq_length - 1:
        ...
        ori_tokens = ori_tokens[0:(max_seq_length - 1)]
    segment_ids = []
    label_ids = []
    qp_label_id = 0
    ntokens = []
    norm_tokens = []
    ntokens.append("[CLS]")
    norm_tokens.append("[CLS]")
    segment_ids.append(0)
    label_ids.append(label_map["[CLS]"])
    query_end_index = -1
    if "#" in tokens:
        query_end_index = tokens.index("#")
    if query_end_index > 0:
        qp_label_id = int(labels[query_end_index])
    for i, token in enumerate(tokens):
        if not i == query_end_index:
            ntokens.append(token)
            if token == "[UNK]":
                norm_tokens.append(ori_tokens[i])
            else:
                norm_tokens.append(token)
            label_ids.append(label_map[labels[i]])
            if i < query_end_index:
                segment_ids.append(0)
            else:
                segment_ids.append(1)
        else:
            ntokens.append("[SEP]")
            norm_tokens.append("[SEP]")
            segment_ids.append(0)
    ...
    for i in range(len(ntokens)):
        if ntokens[i] == "[UNK]" and len(ntokens) == len(norm_tokens):
            #还原UNK token
            ntokens[i] = norm_tokens[i]
    ...
    if ex_index < 3:
        ...
        logging.info("qp_label_id: %s" % (qp_label_id))
    feature = InputFeatures(
        input_ids=input_ids,
        mask=mask,
        segment_ids=segment_ids,
        label_ids=label_ids,
        qp_label_id=qp_label_id
    )
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file,mode=None):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["qp_label_id"] = create_int_feature([feature.qp_label_id])
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        ...
        "qp_label_id": tf.FixedLenFeature([], tf.int64),
...
def create_model(bert_config, is_training, input_ids, mask,
                 segment_ids, qp_label, labels, num_labels, use_one_hot_embeddings, dropout_rate=1.0):
    ...
    #qp相关性
    qp_output_layer = model.get_pooled_output()

    hidden_size = qp_output_layer.shape[-1].value
    ...
    #
    if is_training:
        qp_output_layer = tf.keras.layers.Dropout(rate=0.1)(qp_output_layer)
        output_layer = tf.keras.layers.Dropout(rate=0.1)(output_layer)

    #qp_相关性loss
    qp_logits = tf.matmul(qp_output_layer, output_weights, transpose_b=True)
    qp_logits = tf.nn.bias_add(qp_logits, output_bias)
    qp_probabilities = tf.nn.softmax(qp_logits, axis=-1)
    qp_log_probs = tf.nn.log_softmax(qp_logits, axis=-1)

    qp_one_hot_labels = tf.one_hot(qp_label, depth=2, dtype=tf.float32)

    qp_per_example_loss = -tf.reduce_sum(qp_one_hot_labels * qp_log_probs, axis=-1)

    qp_loss = tf.reduce_mean(qp_per_example_loss)
    ...
    logits = tf.reshape(logits,[-1, 256, num_labels])
    #if FLAGS.crf:
    if True:
        mask2len = tf.reduce_sum(mask, axis=1)
        tagging_loss, trans = crf_loss(logits,labels, mask, num_labels, mask2len)
        predict, viterbi_score = tf.contrib.crf.crf_decode(logits, trans, mask2len)
        model_result = {}
        #loss = FLAGS.qp_loss_rate * qp_loss + (1 - FLAGS.qp_loss_rate) * tagging_loss
        loss = 0.25 * qp_loss + (1 - 0.25) * tagging_loss
        #loss = tagging_loss
        model_result["total_loss"] =  loss
        model_result["predicts"] = predict
        model_result["logits"] = logits
        model_result["qp_probabilities"] = qp_probabilities
        model_result["viterbi_score"] =  viterbi_score
        model_result["qp_output_layer"] =  qp_output_layer
        return model_result
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        qp_label_id = features["qp_label_id"]
        is_training = (mode == tf.estimator.ModeKeys.TRAIN)
        #(total_loss, logits, predicts) = create_model(bert_config, is_training, input_ids,
        model_result = create_model(bert_config, is_training, input_ids,
                                                        mask, segment_ids, qp_label_id, label_ids, num_labels, 
                                                        use_one_hot_embeddings)
        total_loss = model_result["total_loss"]
        #predicts = model_result["predicts"]
        qp_probabilities = model_result["qp_probabilities"]
        predicts = {"predict": model_result["predicts"], "qp_predict": qp_probabilities} 
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            tensors_to_log = {'loss': total_loss} 
...
def _write_base(batch_tokens,id2label,prediction, qp_score, batch_labels,wf,i):
    ...
    if token!="[PAD]" and token!="[CLS]" and true_l!="X":
        ...
        wf.write(line)
    #adapt vi version data
    if token=="[CLS]":
        if i == 0:
            #line = "{}\t{}\t{}\n".format("[CLS]", "[CLS]", "[CLS]")
            line = "{}\t{}\t{}\n".format("[CLS]", "[CLS]", qp_score)
            wf.write(line)
        else:
            line = "{}\t{}\t{}\n".format("[SEP]", "[SEP]", "[SEP]")
            wf.write(line)
...
def Writer(output_predict_file,result, qp_results, batch_tokens,batch_labels,id2label):
    with open(output_predict_file, 'w', encoding="UTF-8") as wf:
        predictions  = []
        for m,pred in enumerate(result):
            #predictions.extend(pred)
            #for i,prediction in enumerate(predictions):
            qp_score = qp_results[m].tolist()[1]
            for i,prediction in enumerate(pred):
                _write_base(batch_tokens[m], id2label, prediction, qp_score, batch_labels[m], wf, i)
            line = "{}\t{}\t{}\n".format("[SEP]", "[SEP]", "[SEP]")
            wf.write(line)
        line = "{}\t{}\t{}\n".format("[SEP]", "[SEP]", "[SEP]")
        wf.write(line)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        result = list(result)
        qp_results = [pred['qp_predict'] for pred in result]
        result = [pred['predict'].tolist() for pred in result]
        #print(type(qp_result))
        #qp_prob = [pred['qp_predict'] for pred in result]
        ...
        Writer(output_predict_file, result, qp_results, batch_tokens,batch_labels,id2label)
